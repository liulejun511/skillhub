...
`usage` 从你**本地的 Claude Code transcript** 统计每个技能触发了多少次(无遥测、不外传):

```bash
PYTHONPATH=tools python -m memoket usage --days 7
```

可以设成每天自动出报告(还能推到飞书)—— 见 [`.kiro/specs/skill-usage-analytics/`](.kiro/specs/skill-usage-analytics/design.md)。
