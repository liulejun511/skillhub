"""晋级 helper：把一个 sandbox 技能晋级成 curated 的**独立插件**（设计 R1.3 / R2.4）。
...
- 在 curated marketplace.json 追加一条 github 源 + commit SHA 钉死的 entry。
...
def promote_skill(name: str, sha: str, repo: str, root: Optional[Path] = None) -> Dict:
    """把 sandbox/skills/<name> 晋级成独立插件 plugins/<name>/。

    返回 {skill, plugin_dir, sha, description}；任何前置条件不满足则抛异常（不留半成品）。
    """
    ...
    # 3) 写插件 manifest（description 即用户在浏览器装前看到的可视化描述）
    ...
    # 4) curated marketplace 追加一条 github 源 + SHA 钉死
    catalog.setdefault("plugins", []).append({
        "name": name,
        "source": {"source": "github", "repo": repo, "sha": sha},
    ...
    return {"skill": name, "plugin_dir": str(plugin_dir), "sha": sha, "description": description}
